...
function readProfiles() {
  try { return JSON.parse(fs.readFileSync(PROF_FILE, 'utf8')); }
  catch { return { profiles: {} }; }
}
function writeProfiles(db) {
  fs.writeFileSync(PROF_FILE, JSON.stringify(db, null, 2));
}
...
app.post('/api/ask/:slug', askLimiter, (req, res) => {
  ...
  const msg = {
    ...
    t: Date.now(),
    text: clean,
    nick: (nickname && nickname.trim().slice(0, 16)) || anonName(),
    role: 'ask'
  };
  db.profiles[slug].messages.push(msg);

  // ✅ 오타 수정 (db.proices → db.profiles[slug].messages)
  if (db.profiles[slug].messages.length > 5000) {
    db.profiles[slug].messages = db.profiles[slug].messages.slice(-3000);
  }
...
server.listen(PORT, () => {
  console.log(`Pushoong-ish Q&A on http://localhost:${PORT}`);
});
